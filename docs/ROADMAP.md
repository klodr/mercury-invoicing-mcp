...
- **Invoice / receipt PDF retrieval** *(bumped to near-term — real demand)* — `mercury_get_invoice` returns the invoice `slug`, but no tool returns the generated PDF document; `mercury_list_invoice_attachments` only lists manually-uploaded files (empty for a normal invoice). Mercury serves the generated PDF from the public pay-page backend — `https://backend.mercury.com/ar-invoice/{slug}/pdf` (invoice) and `https://backend.mercury.com/ar/receipt/{receiptId}/pdf` (receipt), no auth required, identical to the "Download Invoice / Download Receipt" buttons on the hosted pay page. Add read-only `mercury_get_invoice_pdf` (+ `mercury_get_receipt_pdf`) that resolve the slug and return the URL and/or download the bytes. **Two dependencies before coding:** (1) confirm whether `api.mercury.com` exposes an *official, documented* PDF endpoint to prefer over the undocumented `backend.mercury.com` front host (which can change without notice); (2) the SSRF guard in `safe-url.ts` allows only `api.mercury.com` today — wrapping `backend.mercury.com` means widening that allowlist, a security-guard change that requires a matching `docs/ASSURANCE_CASE.md` update. Demand driver: supplying invoice PDFs as supporting documents for bank KYB / compliance replies (e.g. RelayFi wire applications).
...
- **MCP `outputSchema` per tool** — `defineTool()` currently only forwards `inputSchema` to `server.registerTool()`. The MCP spec (2025-06-18+) supports an optional `outputSchema` so clients can validate `structuredContent` and the LLM gets typed return shapes. We already emit `structuredContent` from every handler — only the schema declaration is missing. Scope: extend `defineTool()` with an optional `outputSchema?: ZodRawShape`, then write a Zod schema for each of the ~36 tools. Lets us drop the textual `RETURNS:` block from tool descriptions and rely on a machine-readable contract instead.
