...
- ~~**Invoice PDF retrieval**~~ — shipped: `mercury_get_invoice_pdf` returns the download URL for Mercury's documented [`getinvoicepdf`](https://docs.mercury.com/reference/getinvoicepdf) endpoint (`{baseUrl}/ar/invoices/{id}/pdf`, `Accept: application/pdf`, per the official mercury-go SDK). The URL is derived from the client's configured base URL, so it stays correct under sandbox / proxy hosts (an earlier draft hard-coded the undocumented `backend.mercury.com` pay-page host — dropped after review by CodeRabbit + Codex on #215). **Still open:** returning the PDF *bytes* would need a binary-capable client + a non-text `ToolResult` (today it's text-only); statement PDF (`getstatementpdf`); receipt PDF. Revisit on demand.
