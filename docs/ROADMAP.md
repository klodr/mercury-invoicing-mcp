...
- ~~**Invoice PDF retrieval**~~ — shipped: `mercury_get_invoice_pdf` fetches the invoice, reads its `slug`, and returns the canonical Mercury-hosted PDF download URL (`backend.mercury.com/ar-invoice/{slug}/pdf`) — the same file the pay page's "Download Invoice" button serves. The MCP only *computes* the public URL from the authenticated `GET /ar/invoices/{id}` response, so no bearer token leaves `api.mercury.com` and the SSRF allowlist is unchanged. **Still open:** receipt PDF (`backend.mercury.com/ar/receipt/{receiptId}/pdf`) needs a `receiptId` that the invoice object does not carry; and statement PDF (`getstatementpdf`). Revisit if there is demand, or if Mercury documents an official on-host `getinvoicepdf` we can prefer over the pay-page host.
